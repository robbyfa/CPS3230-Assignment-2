...
if (0==1){}
else if (_state_id_loginProperty==2){
		if (1==0){}
		else if ((_occurredEvent(_event,4/*logout*/)) && (loggedIn ==false )){
		_cls_test0.pw .println ("Logged out \nEvent log type: "+eventLogType );

		_state_id_loginProperty = 3;//moving to state starting
		_goto_loginProperty(_info);
		}
}
else if (_state_id_loginProperty==3){
		if (1==0){}
		else if ((_occurredEvent(_event,6/*invalidLogin*/)) && (loggedIn ==false )){
		_cls_test0.pw .println ("Bad Login");

		_state_id_loginProperty = 0;//moving to state invalidLogin
		_goto_loginProperty(_info);
		}
		else if ((_occurredEvent(_event,0/*validLogin*/)) && (loggedIn ==true )){
		_cls_test0.pw .println ("Login Page. \nEvent log type: "+eventLogType +"\nEmpty: "+empty );

		_state_id_loginProperty = 1;//moving to state login
		_goto_loginProperty(_info);
		}
		else if ((_occurredEvent(_event,2/*viewAlerts*/)) && (loggedIn ==true )){
		_cls_test0.pw .println ("Login Page \nEvent log type: "+eventLogType );
		...
		_state_id_loginProperty = 2;//moving to state alerts
		_goto_loginProperty(_info);
		}
}
else if (_state_id_loginProperty==0){
		if (1==0){}
		else if ((_occurredEvent(_event,0/*validLogin*/)) && (loggedIn ==true )){
		_cls_test0.pw .println ("Login Page. \nEvent log type: "+eventLogType +"\nEmpty: "+empty );
...
else if (_state_id_loginProperty==1){
		if (1==0){}
		else if ((_occurredEvent(_event,2/*viewAlerts*/)) && (loggedIn ==true )){
...
switch(_state_id){
case 2: if (_mode == 0) return "alerts"; else return "alerts";
case 0: if (_mode == 0) return "invalidLogin"; else return "!!!SYSTEM REACHED BAD STATE!!! invalidLogin "+new _BadStateExceptiontest().toString()+" ";
case 1: if (_mode == 0) return "login"; else return "login";
case 3: if (_mode == 0) return "starting"; else return "starting";
default: return "!!!SYSTEM REACHED AN UNKNOWN STATE!!!";
}
}
int _state_id_alertsProperty = 7;
...
if (0==1){}
else if (_state_id_alertsProperty==4){
		if (1==0){}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (empty ==false &&status ==201 )){
		alerts ++;
_cls_test0.pw .println ("Alert created. Number of alerts: "+alerts +". Event log type: "+eventLogType +"\nEmpty: "+empty );

		_state_id_alertsProperty = 5;//moving to state notEmpty
		_goto_alertsProperty(_info);
		}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (status ==400 )){
		_cls_test0.pw .println ("Bad upload. Number of alerts: "+alerts +"\nStatus: "+status );

		_state_id_alertsProperty = 4;//moving to state badUpload
		_goto_alertsProperty(_info);
		}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (alerts >=5 &&status ==201 )){
		alerts ++;
_cls_test0.pw .println ("Alerts full. Number of alerts: "+alerts +"\nStatus: "+status +"\nEventLogType: "+eventLogType );

		_state_id_alertsProperty = 6;//moving to state full
		_goto_alertsProperty(_info);
		}
}
else if (_state_id_alertsProperty==5){
		if (1==0){}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (alerts <5 &&status ==201 )){
		alerts ++;
...
_cls_test0.pw .println ("Alerts full. Number of alerts: "+alerts +"\nStatus: "+status +"\nEventLogType: "+eventLogType );
		...
		else if ((_occurredEvent(_event,18/*deleteAlerts*/)) && (empty ==true &&status ==200 )){
		alerts =0 ;
_cls_test0.pw .println ("Alerts deleted. Event log type: "+eventLogType +"\nEmpty: "+empty );

		_state_id_alertsProperty = 7;//moving to state empty
		_goto_alertsProperty(_info);
		}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (status ==400 )){
		_cls_test0.pw .println ("Bad upload. Number of alerts: "+alerts +"\nStatus: "+status );
...
else if (_state_id_alertsProperty==7){
		if (1==0){}
		else if ((_occurredEvent(_event,16/*createAlert*/)) && (empty ==false &&status ==201 )){
		alerts =0 ;
alerts ++;
...
else if (_state_id_alertsProperty==6){
		if (1==0){}
		else if ((_occurredEvent(_event,18/*deleteAlerts*/)) && (empty ==true &&status ==200 )){
		alerts =0 ;
_cls_test0.pw .println ("Alerts deleted. Event log type: "+eventLogType +"\nEmpty: "+empty );
...
switch(_state_id){
case 4: if (_mode == 0) return "badUpload"; else return "!!!SYSTEM REACHED BAD STATE!!! badUpload "+new _BadStateExceptiontest().toString()+" ";
case 5: if (_mode == 0) return "notEmpty"; else return "notEmpty";
case 6: if (_mode == 0) return "full"; else return "full";
case 7: if (_mode == 0) return "empty"; else return "empty";
default: return "!!!SYSTEM REACHED AN UNKNOWN STATE!!!";
}
}
